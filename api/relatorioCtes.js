...
SELECT L.CHAVE_CTE,
       ...
       NVL(NULLIF(CAB.QTDVOL, 0), NVL(XMLNF.VOLUMES, 0)) VOLUMES,
       L.VALOR_FRETE
FROM LINHAS_CTE L
...
OUTER APPLY (
  SELECT /*+ INDEX(IX TGFIXN_I02) */
         ...
         MAX(DADOS_XML.CIDADE) KEEP (
           DENSE_RANK LAST ORDER BY IX.NUARQUIVO
         ) CIDADE,
         MAX(DADOS_XML.UF) KEEP (
           DENSE_RANK LAST ORDER BY IX.NUARQUIVO
         ) UF,
         MAX(DADOS_XML.PESO) KEEP (
           DENSE_RANK LAST ORDER BY IX.NUARQUIVO
         ) PESO,
         MAX(DADOS_XML.VOLUMES) KEEP (
           DENSE_RANK LAST ORDER BY IX.NUARQUIVO
         ) VOLUMES
  FROM TGFIXN IX
  OUTER APPLY (
    ...
    FROM XMLTABLE(
           '/nfeProc/NFe/infNFe'
           PASSING XMLTYPE(IX.XML)
           COLUMNS CIDADE VARCHAR2(120) PATH 'dest/enderDest/xMun',
                   UF VARCHAR2(2) PATH 'dest/enderDest/UF'
         ) ENDERECO
    OUTER APPLY (
      SELECT SUM(TO_NUMBER(
                   NULLIF(TRIM(VOL.PESO_TEXTO), ''),
                   '999999999999999D999999',
                   'NLS_NUMERIC_CHARACTERS=''.,'''
                 )) PESO,
             SUM(TO_NUMBER(
                   NULLIF(TRIM(VOL.VOLUMES_TEXTO), ''),
                   '999999999999999D999999',
                   'NLS_NUMERIC_CHARACTERS=''.,'''
                 )) VOLUMES
      FROM XMLTABLE(
             ...
             PASSING XMLTYPE(IX.XML)
             COLUMNS PESO_TEXTO VARCHAR2(50) PATH 'pesoB',
                     VOLUMES_TEXTO VARCHAR2(50) PATH 'qVol'
           ) VOL
    ) VOLUME
  ) DADOS_XML
  WHERE IX.CHAVEACESSO = L.CHAVENFE
    AND IX.TIPO = 'N'
    -- XML e CLOB sao processados somente quando o documento nativo nao
    -- possui os dados necessarios. Isso preserva a fonte principal e reduz
    -- muito o custo da exportacao em periodos grandes.
    AND (
      NF.NUNOTA IS NULL
